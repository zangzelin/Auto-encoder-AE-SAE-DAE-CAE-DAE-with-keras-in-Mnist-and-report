...
$$ h_1 = \sigma_e( W_1 x+b_1 )  $$
$$ y = \sigma_d(W_2h_1+b_3)  $$

其中$W_1$,$b_1$为编码的权重和偏置，$W_2$,$b_2$为解码的权重和偏置。

损失函数描述为如下形式:

$$
J(W, b)=\sum(L( x,  y))=\sum\| y- x\|_2^2
$$
...
$$  h_1 = \sigma_e( W_1\hat  x+b_1 )  $$ 
$$ y = \sigma_d(W_2 h_1+b_3) $$

其中$q_d(\hat x | x)$为噪声的分布，$\hat x$ 为加入噪声后的输入， $W_1$,$b_1$为编码的权重和偏置，$W_2$,$b_2$为解码的权重和偏置。

降噪自编码器的目标是最小化损失函数。

$$
J_{D A E}({W})=\sum E_{\tilde{x} \sim q_{D}(\tilde{x} | x)}[L(x, y)] 
$$
...
其中,编码器 $f_1$ 分别对应的是预先训练好的降噪自编码器$D_1$到$D_n$的编码函数.
...
* 1. 干净输入$x$输入
* 2. 加入噪声,和与$f_1$对应的解码器$f'_1$使用降噪自编码器的方式训练$f_1$.
* 3. 固定$f_1$,以与步骤2相同的方式训练$f_2$到$f_n$,并在最后一层加入分类器
* 4. 利用$x$的真实标签和预测获得的标签进行有监督学习.对整个网络参数进行进一步的微调.
...
其中$X$代表了输入数据,$Z$代表了隐变量(latent varibles)(一个类似隐层输出的概率描述),
变分编码器的目标可以描述为最大化$X$生成的概率$P(X)$

$$
P(\boldsymbol{x})=\int P(\boldsymbol{x} | \boldsymbol{z} ; \theta) P(\boldsymbol{z}) \mathrm{d} \boldsymbol{z}
$$

其中 $\theta$ 为模型的参数,也是优化的对象.一般选取$P(\boldsymbol{x} | \boldsymbol{z} ; \theta) = \mathcal{N}\left(\boldsymbol{x} | f(\boldsymbol{z} ; \theta), \sigma^{2} \times \boldsymbol{I}\right)$

loss函数的推导过程:

首先我们希望编码器部分生成的随机变量$Q(z|x)$与理想的$P(z|x)$尽量接近,因此使用KL散度的概念对两个随机变量的相似性进行描述.

$$
\mathcal{D}[Q(z | x) \| P(z | x)]=E_{z \sim Q}[\log (Q(z | x)-\log P(z | x)]
$$

其中,$\mathcal{D}$为散度符号.利用贝叶斯公式将上述公式展开得到:

$${\mathcal{D}[Q(z | x) | P(z | x)]=E_{z \sim Q} [\log (Q(z | x)-}{\quad \log P(x | z)-\log P(z)+\log P(x) ]} $$

经过变换可以得到:

$$
{\log P(\boldsymbol{x})-\mathcal{D}[Q(\boldsymbol{z} | \boldsymbol{x}) \| P(\boldsymbol{z} | \boldsymbol{x})]=} {E_{z \sim \varrho}[\log (\boldsymbol{x}(\boldsymbol{x} | \boldsymbol{z})]-\mathcal{D}[Q(\boldsymbol{z} | \boldsymbol{x}) \| P(\boldsymbol{z})]}
$$
